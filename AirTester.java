...
public class AirTester {
	...
	private static void checkAirport() {
		...
		if (trouble) {
			...
			for (int i = 0; i < 4; i++)
				err += "\n  isConnectedTo(" + checks[i] + ") returned " + that.isConnectedTo(checks[i]) + ", expected "
						+ bExpect[i];
			abortZeroPoints(err);
		}
	}
	...
	private static void checkCompareTo(Airport a1, Airport a2, int expectSignum) {
		...
		if (Math.signum(result) != expectSignum)
			abortZeroPoints("compareTo() on " + a1 + " and " + a2 + " returned " + result + ", should be "
					+ signumToString(expectSignum));

		result = a2.compareTo(a1);
		expectSignum *= -1;
		if (Math.signum(result) != expectSignum)
			abortZeroPoints("compareTo() on " + a2 + " and " + a1 + " returned " + result + ", should be "
					+ signumToString(expectSignum));
	...
	public static void main(String[] args) {
		...
		checkAirport();
		checkFlightNet();
		sop("Full marks for correct code.");
